...
const removerItem = (conviteIndex) => {
    ...
    return (<div className="ListaConvite">
        {convites.map((convite, index,) => (
                <ListaConviteItem  key = {`ListaConviteitem-${index}`} convite = {convite} quantidadeSelecionada = {conviteSelecionado} index = {index}
                ...
                {conviteModal && <DetalhesModal convite= {conviteModal} closeModal={() => setConviteModal (false)}/>}
    </div>);
  }
